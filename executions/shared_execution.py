...
class Shared:
    ...
    def run(self):
        ...
        self.highlight_coding_regions(dna_highlighter, coding_regions)
        ...
        if len(coding_regions) > 0:
            original_coding_regions, selected_regions_to_exclude, selected_region_list = self.handle_coding_region_elimination(dna_highlighter, self.region_list, coding_indexes, coding_regions)
        else:
        ...
        marked_input_seq, marked_target_seq = self.mark_and_print_non_equal_codons(selected_region_list, region_list_target)
        ...
        self.create_report_and_save(target_seq, marked_input_seq, marked_target_seq, original_coding_regions, selected_regions_to_exclude, self.region_list, selected_region_list, min_cost)
    ...
    def create_report_and_save(self, target_seq, marked_input_seq, marked_target_seq, regions, chosen_regions, region_list, selected_region_list, min_cost):
        # Create a report summarizing the processing and save if the user chooses to
        Report(self.seq, target_seq, marked_input_seq, marked_target_seq, self.unwanted_patterns, regions, chosen_regions, region_list, selected_region_list, min_cost).create_report()
