...
def get_elimination_process_description():
    return (
        "Elimination process executed with the following cost parameters:"
    )
...
def get_non_coding_region_cost_description():
    return (
        f" • Transition substitution in non-coding region (A ↔ G, C ↔ T): α = {format_cost(CostData.alpha)}\n"
        f" • Transversion substitution in non-coding region ({{A,G}} ↔ {{C,T}}): β = {format_cost(CostData.beta)}"
    )

def get_info_usage():
    lines = [
        "Note that the open reading frames (ORFs) in the specified target sequence",
...
def get_elimination_info():
    lines = [
        ...
        " • If the substitution is a transition (A ↔ G or C ↔ T), a lower cost of ⍺ is applied.",
        ...
        "   frequency from user-specified codon usage table.",
